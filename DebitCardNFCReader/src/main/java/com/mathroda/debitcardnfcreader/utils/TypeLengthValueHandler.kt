...
import com.mathroda.debitcardnfcreader.model.TagAndLength
import com.mathroda.debitcardnfcreader.model.TypeLengthValue
import com.mathroda.debitcardnfcreader.model.commandhelper.DebitCardTags
import com.mathroda.debitcardnfcreader.utils.ByteUtils.byteArrayToInt
import com.mathroda.debitcardnfcreader.utils.ByteUtils.matchBitByBitIndex
import java.io.ByteArrayInputStream
import java.io.ByteArrayOutputStream

internal object TypeLengthValueHandler {
    ...
    private fun getNextTLV(stream: ByteArrayInputStream): TypeLengthValue? {
        ...
        if (lengthBytes.isEmpty() || lengthBytes.size > 4) { //Number of length bytes must be from 1 to 4
            return null
        }
        stream.read(lengthBytes, 0, lengthBytes.size)
        val rawLength = byteArrayToInt(lengthBytes)
        val valueBytes: ByteArray
        val tag = DebitCardTags.find(tagIdBytes)
        ...
        return TypeLengthValue(tag, length, lengthBytes, valueBytes)
    }
    ...
    fun parseTagAndLength(data: ByteArray?): List<TagAndLength> {
        ...
        if (data != null) {
            ...
            while (stream.available() > 1) {
                val tag = DebitCardTags.find(readTagIdBytes(stream))
     ...
     */
    fun getlistTLV(data: ByteArray?, vararg tags: DebitCardTag?): List<TypeLengthValue?> {
        val list: MutableList<TypeLengthValue?> = ArrayList()
        if(data != null) {
            val stream = ByteArrayInputStream(data)
     ...
     */
    fun getValue(data: ByteArray?, vararg tags: DebitCardTag?): ByteArray? {
